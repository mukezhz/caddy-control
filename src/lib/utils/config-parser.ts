          ...
          matchBlock.host.forEach(host => {
            try {
              ...
              if (isRedirect) {
                ...
                if (locationHeader) {
                  ...
                  extractedDomains.push({
                    ...
                    enableHttps: true,
                    isValid: false,
                    errorMessage: "Redirect location header not found"
                  });
                }
              } else {
                  ...
                  if (upstreams && upstreams.length > 0) {
                    ...
                    if (upstream.dial) {
                      ...
                      extractedDomains.push({
                        ...
                        enableHttps: true,
                        isValid: false,
                        errorMessage: "Reverse proxy dial string not found"
                      });
                    }
                  } else {
                    extractedDomains.push({
                      ...
                      enableHttps: true,
                      isValid: false,
                      errorMessage: "Reverse proxy upstreams not found"
                    });
                  }
                } else {
                  extractedDomains.push({
                    ...
                    enableHttps: true,
                    isValid: false,
                    errorMessage: "No valid handler found (neither redirect nor reverse proxy)"
                  });
                }
              }
            } catch (error) {
              extractedDomains.push({
                ...
                enableHttps: true,
                isValid: false,
                errorMessage: "Failed to parse route configuration" + ((error as Error)?.message || "")
              });
            }
          });
        }
